...
export class UniSwapPairTracker {
  ...
  async start() {
    ...
    const token0 = await Fetcher.fetchTokenData(
      this.chain,
      utils.getAddress(tokens[this.chain][this.pair[0]])
    );
    console.log("Fetching Token Data of ", this.pair[1]);
    const token1 = await Fetcher.fetchTokenData(
      this.chain,
      utils.getAddress(tokens[this.chain][this.pair[1]])
    );
    ...
    while (true) {
      try {
        ...
        this.eventservice.write({
          type: "price",
          pair: pairinstring,
          price: p.toSignificant(8),
        });
